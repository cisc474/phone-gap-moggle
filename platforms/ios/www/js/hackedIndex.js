...
onDeviceReady: function() {
    ...
    var getChildContainer = function(sEvent, mX, mY){
        var ans = -1;
        //alert(sEvent.children.length.toString() + mX.toString() + mY.toString());
        //alert(sEvent.getObjectsUnderPoint(mX, mY));
        _.each(sEvent.children, function(dObject){
               //                  var pt = dObject.globalToLocal(mX, mY);
               var bnds = dObject.getBounds();
               //                   alert(" "+ bnds.width + "  x " + bnds.height + " at "+  dObject.x + " " + dObject.y + " clicked "+mX + " " + mY);
               if ((mX >= dObject.x) && (mX <= (bnds.width*2 + dObject.x)) && (mY >= dObject.y) && (mY <= (bnds.height*2 + dObject.y))){
               ans = dObject.id;
               //alert("found hit "+dObject.id);
               }
               //else { alert("missed "+mX + " "+mY + " ");}
               //alert(dObject.hitTest(pt.x, pt.y)?"hit "+dObject.id:"missed "+mX+" "+mY+" "+dObject.getBounds().width+" wide " + dObject.x + " " +dObject.getBounds().height+" tall at "+dObject.y);
    ...
    stage.addEventListener('stagemousemove', function(event){
                           var stageEvent = event.target;
                           var aTarget = getChildContainer(stageEvent, event.rawX, event.rawY);
                           if (aTarget != -1){
                           ui.containerArray[aTarget].trigger('myenter');
                           } else {
                           game.trigger('myexit');
                           }
                           });
